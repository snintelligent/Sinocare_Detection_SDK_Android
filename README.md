...
```gradle
dependencies {
    implementation 'com.sinocare.androidx_lib:multicriteriasdk:2.2.8_lite'
}
```
...
| 01 | 获取版本号 | 0E | 历史数据补发 |
| 02 | 错误值 | 0C | 获取时间 |
| 04 | 当前测试值（实时数据） | H01 | 通信测试 |
| 05 | 历史数据值 | 11 | TLV数据（随机、餐前、餐后、声音关、声音小、声音中、声音大） |
| 20 | 历史数据序号 | | |
| B7 | 测量过程值 | | |
...
| Type | 说明 | Type | 说明 |
|------|------|------|------|
| bloodGlucose | 血糖 | ferritin | 铁蛋白 |
| bloodLipids | 血脂 | HbA1c | 糖化血红蛋白 |
| bloodPressure | 血压 | Crea | 肌酐 |
| uricAcid | 尿酸 | HGB | 血红蛋白 |
| bloodKetone | 血酮 | HeightAndWeight | 身高体重体脂 |
| urinalysis | 尿常规 | multipleTypes | 多类型指标 |
| bloodOxygen | 血氧 | lac | 血乳酸 |
